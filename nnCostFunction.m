...
% reassigning y(i) to be a vector
...
for i = 1:m
  for k = 1:num_labels
    tmp = (-Y(i,k)*log(h(i,k)) - (1 - Y(i,k))*log(1 - h(i,k)));
    J = J + (1/m)*tmp;
    % J can be written as (Vectorized form) without using nested loops
...
for i = 1:m
  ...
  DELTA2 = DELTA2 + delta2(2:end)'*X(i,:);
